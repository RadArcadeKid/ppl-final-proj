...
object LettuceConsole {
    ...
    def readOneProgram(): (Boolean, String, Int) = {
        ...
        if (debug){
            ...
            if(!quitting){
                println(s" -- STEPPING TO n = $breakN")git
            }
    ...
    def main(args: Array[String]): Unit = {
        ...
        print("------------------------------------------------------------- \n \n")
        val programString = "let x = 3 in let y = 5 in x + y;;"

        val testExpressBreak = "let x  = 3 in _"
        val p: Program = new LettuceParser().parseString(testExpressBreak)
//        val pS = parseString(testExpressBreak)
        println(p)
        while (true){
            print("\n -- Lettuce Program:\n|")
            try {
